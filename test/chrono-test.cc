...
#include "util.hpp"       // get_locale

using lll::fmt::runtime;
using lll::fmt::sys_time;
...
template <typename Time> void test_time(Time time) {
  EXPECT_EQ(lll::fmt::format("{}", time), "1979-03-12 12:00:00");
  EXPECT_EQ(lll::fmt::format("{:}", time), "1979-03-12 12:00:00");

  EXPECT_EQ(lll::fmt::format("{:%%}", time), "%");
  EXPECT_EQ(lll::fmt::format("{:%n}", time), "\n");
  EXPECT_EQ(lll::fmt::format("{:%t}", time), "\t");
  EXPECT_EQ(lll::fmt::format("{:%Y}", time), "1979");
  EXPECT_EQ(lll::fmt::format("{:%EY}", time), "1979");
  EXPECT_EQ(lll::fmt::format("{:%y}", time), "79");
  EXPECT_EQ(lll::fmt::format("{:%Oy}", time), "79");
  EXPECT_EQ(lll::fmt::format("{:%Ey}", time), "79");
  EXPECT_EQ(lll::fmt::format("{:%C}", time), "19");
  EXPECT_EQ(lll::fmt::format("{:%EC}", time), "19");
  EXPECT_EQ(lll::fmt::format("{:%G}", time), "1979");
  EXPECT_EQ(lll::fmt::format("{:%g}", time), "79");
  EXPECT_EQ(lll::fmt::format("{:%b}", time), "Mar");
  EXPECT_EQ(lll::fmt::format("{:%h}", time), "Mar");
  EXPECT_EQ(lll::fmt::format("{:%B}", time), "March");
  EXPECT_EQ(lll::fmt::format("{:%m}", time), "03");
  EXPECT_EQ(lll::fmt::format("{:%Om}", time), "03");
  EXPECT_EQ(lll::fmt::format("{:%U}", time), "10");
  EXPECT_EQ(lll::fmt::format("{:%OU}", time), "10");
  EXPECT_EQ(lll::fmt::format("{:%W}", time), "11");
  EXPECT_EQ(lll::fmt::format("{:%OW}", time), "11");
  EXPECT_EQ(lll::fmt::format("{:%V}", time), "11");
  EXPECT_EQ(lll::fmt::format("{:%OV}", time), "11");
  EXPECT_EQ(lll::fmt::format("{:%j}", time), "071");
  EXPECT_EQ(lll::fmt::format("{:%d}", time), "12");
  EXPECT_EQ(lll::fmt::format("{:%Od}", time), "12");
  EXPECT_EQ(lll::fmt::format("{:%e}", time), "12");
  EXPECT_EQ(lll::fmt::format("{:%Oe}", time), "12");
  EXPECT_EQ(lll::fmt::format("{:%a}", time), "Mon");
  EXPECT_EQ(lll::fmt::format("{:%A}", time), "Monday");
  EXPECT_EQ(lll::fmt::format("{:%w}", time), "1");
  EXPECT_EQ(lll::fmt::format("{:%Ow}", time), "1");
  EXPECT_EQ(lll::fmt::format("{:%u}", time), "1");
  EXPECT_EQ(lll::fmt::format("{:%Ou}", time), "1");
  EXPECT_EQ(lll::fmt::format("{:%H}", time), "12");
  EXPECT_EQ(lll::fmt::format("{:%OH}", time), "12");
  EXPECT_EQ(lll::fmt::format("{:%I}", time), "12");
  EXPECT_EQ(lll::fmt::format("{:%OI}", time), "12");
  EXPECT_EQ(lll::fmt::format("{:%M}", time), "00");
  EXPECT_EQ(lll::fmt::format("{:%OM}", time), "00");
  EXPECT_EQ(lll::fmt::format("{:%S}", time), "00");
  EXPECT_EQ(lll::fmt::format("{:%OS}", time), "00");
  EXPECT_EQ(lll::fmt::format("{:%x}", time), "03/12/79");
  EXPECT_EQ(lll::fmt::format("{:%Ex}", time), "03/12/79");
  EXPECT_EQ(lll::fmt::format("{:%X}", time), "12:00:00");
  EXPECT_EQ(lll::fmt::format("{:%EX}", time), "12:00:00");
  EXPECT_EQ(lll::fmt::format("{:%D}", time), "03/12/79");
  EXPECT_EQ(lll::fmt::format("{:%F}", time), "1979-03-12");
  EXPECT_EQ(lll::fmt::format("{:%R}", time), "12:00");
  EXPECT_EQ(lll::fmt::format("{:%T}", time), "12:00:00");
  EXPECT_EQ(lll::fmt::format("{:%p}", time), "PM");
  EXPECT_EQ(lll::fmt::format("{:%c}", time), "Mon Mar 12 12:00:00 1979");
  EXPECT_EQ(lll::fmt::format("{:%Ec}", time), "Mon Mar 12 12:00:00 1979");
  EXPECT_EQ(lll::fmt::format("{:%r}", time), "12:00:00 PM");

  EXPECT_EQ(lll::fmt::format("{:%Y-%m-%d %H:%M:%S}", time),
            "1979-03-12 12:00:00");
}

TEST(chrono_test, sys_time) {
  auto time =
      lll::fmt::sys_time<std::chrono::seconds>(std::chrono::seconds(290088000));
  test_time(time);
  EXPECT_EQ(lll::fmt::format("{:%z}", time), "+0000");
  EXPECT_EQ(lll::fmt::format("{:%Ez}", time), "+00:00");
  EXPECT_EQ(lll::fmt::format("{:%Oz}", time), "+00:00");
  EXPECT_EQ(lll::fmt::format("{:%Z}", time), "UTC");
}

TEST(chrono_test, local_time) {
  auto time = lll::fmt::local_time<std::chrono::seconds>(
      std::chrono::seconds(290088000));
  test_time(time);
  EXPECT_THROW_MSG((void)lll::fmt::format(lll::fmt::runtime("{:%z}"), time),
                   lll::fmt::format_error, "no timezone");
  EXPECT_THROW_MSG((void)lll::fmt::format(lll::fmt::runtime("{:%Z}"), time),
                   lll::fmt::format_error, "no timezone");
}

template <typename T,
          LAWS3_FMT_ENABLE_IF(lll::fmt::detail::has_tm_gmtoff<T>::value)>
...
TEST(chrono_test, tm) {
  auto time = lll::fmt::gmtime(290088000);
  test_time(time);
  if (set_tm_gmtoff(time, -28800)) {
    EXPECT_EQ(lll::fmt::format(lll::fmt::runtime("{:%z}"), time), "-0800");
    EXPECT_EQ(lll::fmt::format(lll::fmt::runtime("{:%Ez}"), time), "-08:00");
    EXPECT_EQ(lll::fmt::format(lll::fmt::runtime("{:%Oz}"), time), "-08:00");
  } else {
    EXPECT_THROW_MSG((void)lll::fmt::format(lll::fmt::runtime("{:%z}"), time),
                     lll::fmt::format_error, "no timezone");
  }
  char tz[] = "EET";
  if (lll::fmt::detail::set_tm_zone(time, tz)) {
    EXPECT_EQ(lll::fmt::format(lll::fmt::runtime("{:%Z}"), time), "EET");
  } else {
    EXPECT_THROW_MSG((void)lll::fmt::format(lll::fmt::runtime("{:%Z}"), time),
                     lll::fmt::format_error, "no timezone");
  }
...
TEST(chrono_test, daylight_savings_time_end) {
  ...
  auto t = lll::fmt::local_time<std::chrono::seconds>(
      std::chrono::seconds(1729998300));
  EXPECT_EQ(lll::fmt::format("{}", t), "2024-10-27 03:05:00");
}
...
TEST(chrono_test, format_specs) {
  ...
  EXPECT_EQ(lll::fmt::format("{:%j}", days(12)), "12");
...
TEST(chrono_test, weekday) {
  ...
  EXPECT_EQ(lll::fmt::format("{:%a}", sat), "Sat");
  EXPECT_EQ(lll::fmt::format("{:%A}", sat), "Saturday");
  ...
  if (loc != std::locale::classic()) {
    ...
    EXPECT_THAT(saturdays, Contains(lll::fmt::format(loc, "{:L%a}", sat)));
    EXPECT_THAT(saturdays, Contains(lll::fmt::format(loc, "{:L%a}", tm)));
  }
}
...
TEST(chrono_test, cpp20_duration_subsecond_support) {
  ...
  EXPECT_EQ(lll::fmt::format("{:.6%S}", std::chrono::milliseconds{-1234}),
            "-01.234000");
  EXPECT_EQ(lll::fmt::format("{:.2%S}", std::chrono::milliseconds{12345}),
            "12.34");
  EXPECT_EQ(lll::fmt::format("{:.2%S}", std::chrono::milliseconds{12375}),
            "12.37");
  EXPECT_EQ(lll::fmt::format("{:.2%S}", std::chrono::milliseconds{-12375}),
            "-12.37");
  EXPECT_EQ(lll::fmt::format("{:.0%S}", std::chrono::milliseconds{12054}),
            "12");
  EXPECT_EQ(lll::fmt::format("{:.2%S}", std::chrono::milliseconds{99999}),
            "39.99");
  EXPECT_EQ(lll::fmt::format("{:.2%S}", std::chrono::milliseconds{1000}),
            "01.00");
  EXPECT_EQ(lll::fmt::format("{:.3%S}", std::chrono::milliseconds{1}),
            "00.001");
...
TEST(chrono_test, timestamp_sub_seconds) {
  ...
  EXPECT_EQ(lll::fmt::format("{:%T}", t8), "00:00:00.123456789");

  auto t9 =
      sys_time<std::chrono::milliseconds>(std::chrono::milliseconds(2000));
  EXPECT_EQ(lll::fmt::format("{:%S}", t9), "02.000");
...
TEST(chrono_test, glibc_extensions) {
...

  {
    auto t = std::tm();
    t.tm_yday = 7;
    EXPECT_EQ(lll::fmt::format("{:%U,%W,%V}", t), "02,01,01");
    EXPECT_EQ(lll::fmt::format("{:%_U,%_W,%_V}", t), " 2, 1, 1");
    EXPECT_EQ(lll::fmt::format("{:%-U,%-W,%-V}", t), "2,1,1");

    EXPECT_EQ(lll::fmt::format("{:%j}", t), "008");
    EXPECT_EQ(lll::fmt::format("{:%_j}", t), "  8");
    EXPECT_EQ(lll::fmt::format("{:%-j}", t), "8");
  }

  {
    auto t = std::tm();
    t.tm_mday = 7;
    EXPECT_EQ(lll::fmt::format("{:%d}", t), "07");
    EXPECT_EQ(lll::fmt::format("{:%_d}", t), " 7");
    EXPECT_EQ(lll::fmt::format("{:%-d}", t), "7");

    EXPECT_EQ(lll::fmt::format("{:%e}", t), " 7");
  }

  {
    auto t = std::tm();
    t.tm_year = 7 - 1900;
    EXPECT_EQ(lll::fmt::format("{:%Y}", t), "0007");
    EXPECT_EQ(lll::fmt::format("{:%_Y}", t), "   7");
    EXPECT_EQ(lll::fmt::format("{:%-Y}", t), "7");
  }

  {
    auto t = std::tm();
    t.tm_year = -5 - 1900;
    EXPECT_EQ(lll::fmt::format("{:%Y}", t), "-005");
    EXPECT_EQ(lll::fmt::format("{:%_Y}", t), "  -5");
    EXPECT_EQ(lll::fmt::format("{:%-Y}", t), "-5");
  }

  {
    auto t = std::tm();
    t.tm_mon = 7 - 1;
    EXPECT_EQ(lll::fmt::format("{:%m}", t), "07");
    EXPECT_EQ(lll::fmt::format("{:%_m}", t), " 7");
    EXPECT_EQ(lll::fmt::format("{:%-m}", t), "7");
  }
}
...
TEST(chrono_test, year_month_day) {
  ...
  auto year = lll::fmt::year(2024);
  auto month = lll::fmt::month(1);
  auto day = lll::fmt::day(1);
  auto ymd = lll::fmt::year_month_day(year, month, day);

  EXPECT_EQ(lll::fmt::format("{}", year), "2024");
  EXPECT_EQ(lll::fmt::format("{:%Y}", year), "2024");
  EXPECT_EQ(lll::fmt::format("{:%y}", year), "24");

  EXPECT_EQ(lll::fmt::format("{}", month), "Jan");
  EXPECT_EQ(lll::fmt::format("{:%m}", month), "01");
  EXPECT_EQ(lll::fmt::format("{:%b}", month), "Jan");
  EXPECT_EQ(lll::fmt::format("{:%B}", month), "January");

  EXPECT_EQ(lll::fmt::format("{}", day), "01");
  EXPECT_EQ(lll::fmt::format("{:%d}", day), "01");

  EXPECT_EQ(lll::fmt::format("{}", ymd), "2024-01-01");
  EXPECT_EQ(lll::fmt::format("{:%Y-%m-%d}", ymd), "2024-01-01");
  EXPECT_EQ(lll::fmt::format("{:%Y-%b-%d}", ymd), "2024-Jan-01");
  EXPECT_EQ(lll::fmt::format("{:%Y-%B-%d}", ymd), "2024-January-01");

  if (loc != std::locale::classic()) {
    auto months = std::vector<std::string>{"ene.", "ene"};
    EXPECT_THAT(months, Contains(lll::fmt::format(loc, "{:L}", month)));
    EXPECT_THAT(months, Contains(lll::fmt::format(loc, "{:L%b}", month)));
  }
}
