...
TEST(compile_test, output_iterators) {
  std::list<char> out;
  lll::fmt::format_to(std::back_inserter(out), LAWS3_FMT_COMPILE("{}"), 42);
  EXPECT_EQ("42", std::string(out.begin(), out.end()));

  std::stringstream s;
  lll::fmt::format_to(std::ostream_iterator<char>(s), LAWS3_FMT_COMPILE("{}"),
                      42);
  EXPECT_EQ("42", s.str());

  std::stringstream s2;
  lll::fmt::format_to(std::ostreambuf_iterator<char>(s2),
                      LAWS3_FMT_COMPILE("{}.{:06d}"), 42, 43);
  EXPECT_EQ("42.000043", s2.str());
}

#  if LAWS3_FMT_USE_CONSTEVAL && \
      (!LAWS3_FMT_MSC_VERSION || LAWS3_FMT_MSC_VERSION >= 1940)
...
TEST(compile_test, unknown_format_fallback) {
  ...
  std::vector<char> v1;
  lll::fmt::format_to(std::back_inserter(v1), LAWS3_FMT_COMPILE("{}"), 42);
  EXPECT_EQ("42", lll::fmt::string_view(v1.data(), v1.size()));

  std::vector<char> v2;
  lll::fmt::format_to(std::back_inserter(v2), LAWS3_FMT_COMPILE("{name:^4}"),
                      lll::fmt::arg("name", 42));
  EXPECT_EQ(" 42 ", lll::fmt::string_view(v2.data(), v2.size()));
...
TEST(compile_test, print) {
  ...
  EXPECT_WRITE(stderr,
               ...
               "Don't panic!");
  lll::fmt::print(LAWS3_FMT_COMPILE("{}"), std_context_test());
}
#endif
...
template <typename S> auto check_is_compiled_string(const S&) -> bool {
  return lll::fmt::is_compiled_string<S>::value;
}

TEST(compile_test, is_compiled_string) {
  EXPECT_TRUE(check_is_compiled_string(LAWS3_FMT_COMPILE("asdf")));
  EXPECT_TRUE(check_is_compiled_string(LAWS3_FMT_COMPILE("{}")));
}
#endif
...
TEST(compile_time_formatting_test, integer) {
  ...
  EXPECT_EQ("   42", test_format<6>(LAWS3_FMT_COMPILE("{:5}"), 42l));
