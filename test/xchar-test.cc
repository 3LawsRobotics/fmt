...
TEST(xchar_test, format) {
  EXPECT_EQ(lll::fmt::format(L"{}", 42), L"42");
  EXPECT_EQ(lll::fmt::format(L"{}", 4.2), L"4.2");
  EXPECT_EQ(lll::fmt::format(L"{}", L"abc"), L"abc");
  EXPECT_EQ(lll::fmt::format(L"{}", L'z'), L"z");
  EXPECT_THROW(lll::fmt::format(lll::fmt::runtime(L"{:*\x343E}"), 42),
               lll::fmt::format_error);
  EXPECT_EQ(lll::fmt::format(L"{}", true), L"true");
  EXPECT_EQ(lll::fmt::format(L"{0}", L'a'), L"a");
  EXPECT_EQ(lll::fmt::format(L"Letter {}", L'\x40e'), L"Letter \x40e");  // Ў
  if (sizeof(wchar_t) == 4)
    EXPECT_EQ(lll::fmt::format(lll::fmt::runtime(L"{:𓀨>3}"), 42), L"𓀨42");
  EXPECT_EQ(lll::fmt::format(L"{}c{}", L"ab", 1), L"abc1");
}
...
TEST(xchar_test, compile_time_string_format_to) {
  std::wstring ws;
  lll::fmt::format_to(std::back_inserter(ws), LAWS3_FMT_STRING(L"{}"), 42);
  EXPECT_EQ(L"42", ws);
}
...
auto operator<<(std::wostream& os,
                streamable_and_unformattable) -> std::wostream& {
  return os << L"foo";
}
...
TEST(xchar_test, chrono) {
  ...
  EXPECT_EQ(lll::fmt::format("{:%Y-%m-%d %H:%M:%S}", t), "1979-03-12 12:00:00");
}
...
TEST(std_test_xchar, format_bitset) {
  ...
  EXPECT_EQ(lll::fmt::format(L"{}", bs), L"101010");
  EXPECT_EQ(lll::fmt::format(L"{:0>8}", bs), L"00101010");
  EXPECT_EQ(lll::fmt::format(L"{:-^12}", bs), L"---101010---");
}

TEST(std_test_xchar, complex) {
  auto s = lll::fmt::format(L"{}", std::complex<double>(1, 2));
  EXPECT_EQ(s, L"(1+2i)");
  EXPECT_EQ(lll::fmt::format(L"{:.2f}", std::complex<double>(1, 2)),
            L"(1.00+2.00i)");
  EXPECT_EQ(lll::fmt::format(L"{:8}", std::complex<double>(1, 2)), L"(1+2i)  ");
  EXPECT_EQ(lll::fmt::format(L"{:-<8}", std::complex<double>(1, 2)),
            L"(1+2i)--");
}
