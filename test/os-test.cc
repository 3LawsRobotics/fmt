...
TEST(ostream_test, move) {
  ...
  lll::fmt::ostream out = lll::fmt::output_file(test_file);
...
TEST(ostream_test, buffer_size) {
  auto test_file = uniq_file_name(__LINE__);
  lll::fmt::ostream out =
      lll::fmt::output_file(test_file, lll::fmt::buffer_size = 1);
  out.print("{}", "foo");
  out.close();
  file in(test_file, file::RDONLY);
  EXPECT_READ(in, "foo");
...
TEST(ostream_test, flush) {
  auto test_file = uniq_file_name(__LINE__);
  auto out = lll::fmt::output_file(test_file);
  out.print("x");
  out.flush();
  auto in = lll::fmt::file(test_file, file::RDONLY);
  EXPECT_READ(in, "x");
}
...
TEST(file_test, io_putting) {
  ...
  for (int i = 0; i <= 20000; ++i) {
    auto s = lll::fmt::format("{}\n", i);
    lll::fmt::print(write_end.get(), "{}", s);
    write_size += s.size();
  }
