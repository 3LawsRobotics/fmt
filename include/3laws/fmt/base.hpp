...
#define LAWS3_FMT_VERSION 100202
...
#elif LAWS3_FMT_GCC_VERSION >= 600 && LAWS3_FMT_CPLUSPLUS >= 201402L
...
#if defined(LAWS3_FMT_USE_NONTYPE_TEMPLATE_ARGS)
// Use the provided definition.
#elif defined(__NVCOMPILER)
#  define LAWS3_FMT_USE_NONTYPE_TEMPLATE_ARGS 0
#elif LAWS3_FMT_GCC_VERSION >= 903 && LAWS3_FMT_CPLUSPLUS >= 201709L
#  define LAWS3_FMT_USE_NONTYPE_TEMPLATE_ARGS 0
#elif defined(__cpp_nontype_template_args) && \
    __cpp_nontype_template_args >= 201911L
#  define LAWS3_FMT_USE_NONTYPE_TEMPLATE_ARGS 1
#else
#  define LAWS3_FMT_USE_NONTYPE_TEMPLATE_ARGS 0
#endif

// Check if exceptions are disabled.
#ifdef LAWS3_FMT_EXCEPTIONS
// Use the provided definition.
#elif defined(__GNUC__) && !defined(__EXCEPTIONS)
#  define LAWS3_FMT_EXCEPTIONS 0
#elif LAWS3_FMT_MSC_VERSION && !_HAS_EXCEPTIONS
#  define LAWS3_FMT_EXCEPTIONS 0
#else
#  define LAWS3_FMT_EXCEPTIONS 1
#endif
...
#if LAWS3_FMT_HAS_CPP_ATTRIBUTE(noreturn) && LAWS3_FMT_EXCEPTIONS && \
    !LAWS3_FMT_MSC_VERSION && !defined(__NVCC__)
#  define LAWS3_FMT_NORETURN [[noreturn]]
#else
#  define LAWS3_FMT_NORETURN
#endif

#ifndef LAWS3_FMT_NODISCARD
#  if LAWS3_FMT_HAS_CPP17_ATTRIBUTE(nodiscard)
...
#endif
// A version of LAWS3_FMT_INLINE to prevent code bloat in debug mode.
#ifdef NDEBUG
...
#if !defined(LAWS3_FMT_HEADER_ONLY) && defined(_WIN32)
...
#  define LAWS3_FMT_UNICODE !LAWS3_FMT_MSC_VERSION
#endif

#define LAWS3_FMT_FWD(...) static_cast<decltype(__VA_ARGS__)&&>(__VA_ARGS__)

// Enable minimal optimizations for more compact code in debug mode.
LAWS3_FMT_GCC_PRAGMA("GCC push_options")
#if !defined(__OPTIMIZE__) && !defined(__CUDACC__)
LAWS3_FMT_GCC_PRAGMA("GCC optimize(\"Og\")")
#endif
...
LAWS3_FMT_CONSTEXPR inline auto is_utf8() -> bool {
  LAWS3_FMT_MSC_WARNING(suppress : 4566)
  constexpr unsigned char section[] = "\u00A7";
  // Avoid an MSVC sign extension bug: https://github.com/fmtlib/fmt/pull/2297.
  using uchar = unsigned char;
  return LAWS3_FMT_UNICODE ||
         (sizeof(section) == 3 && uchar(section[0]) == 0xC2 &&
          uchar(section[1]) == 0xA7);
}

template <typename Char>
LAWS3_FMT_CONSTEXPR auto length(const Char* s) -> size_t {
  size_t len = 0;
  while (*s++) ++len;
  return len;
}
...
template <typename Char>
LAWS3_FMT_CONSTEXPR auto compare(const Char* s1, const Char* s2, std::size_t n)
    -> int {
  for (; n != 0; ++s1, ++s2, --n) {
    if (*s1 < *s2) return -1;
    if (*s1 > *s2) return 1;
...
/**
  ...
  even if ``std::string_view`` is available to prevent issues when a library is
...
LAWS3_FMT_DEPRECATED LAWS3_FMT_NORETURN inline void throw_format_error(
    const char* message) {
...
template <typename Char> class basic_format_parse_context {
 private:
  ...
  LAWS3_FMT_CONSTEXPR void do_check_arg_id(int id);
...
class compile_parse_context : public basic_format_parse_context<Char> {
 ...
 public:
  explicit LAWS3_FMT_CONSTEXPR compile_parse_context(
...
/**
  ...
  class and shouldn't be used directly, only via
  `~lll::fmt::basic_memory_buffer`. \endrst
 */
template <typename T> class buffer {
 ...
 protected:
  ...
  LAWS3_FMT_CONSTEXPR20 buffer(grow_fun grow, size_t sz) noexcept
...
template <typename Char>
LAWS3_FMT_CONSTEXPR void basic_format_parse_context<Char>::do_check_arg_id(
    int id) {
  // Argument id is only checked at compile-time during parsing because
  // formatting has its own validation.
  if (detail::is_constant_evaluated() &&
      (!LAWS3_FMT_GCC_VERSION || LAWS3_FMT_GCC_VERSION >= 1200)) {
...
template <typename Char>
LAWS3_FMT_CONSTEXPR void basic_format_parse_context<Char>::check_dynamic_spec(
    int arg_id) {
  if (detail::is_constant_evaluated() &&
      (!LAWS3_FMT_GCC_VERSION || LAWS3_FMT_GCC_VERSION >= 1200)) {
    using context = detail::compile_parse_context<Char>;
...
LAWS3_FMT_EXPORT template <typename Context> class basic_format_arg;
LAWS3_FMT_EXPORT template <typename Context> class basic_format_args;
...
template <typename T> class basic_appender {
 ...
 public:
  ...
  using pointer = T*;
  using reference = T&;
  LAWS3_FMT_UNCHECKED_ITERATOR(basic_appender);

  LAWS3_FMT_CONSTEXPR basic_appender(detail::buffer<T>& buf) : buffer_(&buf) {}
...
template <typename T, typename InputIt, typename OutputIt,
          LAWS3_FMT_ENABLE_IF(!is_back_insert_iterator<OutputIt>::value)>
LAWS3_FMT_CONSTEXPR auto copy(InputIt begin, InputIt end, OutputIt out)
    -> OutputIt {
...
template <typename T>
LAWS3_FMT_CONSTEXPR auto copy(const T* begin, const T* end, T* out) -> T* {
...
LAWS3_FMT_CONSTEXPR auto copy(basic_string_view<V> s, OutputIt out)
    -> OutputIt {
...
template <typename Context> class value {
 public:
  ...
  constexpr LAWS3_FMT_ALWAYS_INLINE value() : no_value() {}
  constexpr LAWS3_FMT_ALWAYS_INLINE value(int val) : int_value(val) {}
  constexpr LAWS3_FMT_ALWAYS_INLINE value(unsigned val) : uint_value(val) {}
  constexpr LAWS3_FMT_ALWAYS_INLINE value(long long val)
      : long_long_value(val) {}
  constexpr LAWS3_FMT_ALWAYS_INLINE value(unsigned long long val)
      : ulong_long_value(val) {}
  LAWS3_FMT_ALWAYS_INLINE value(int128_opt val) : int128_value(val) {}
  LAWS3_FMT_ALWAYS_INLINE value(uint128_opt val) : uint128_value(val) {}
  constexpr LAWS3_FMT_ALWAYS_INLINE value(float val) : float_value(val) {}
  constexpr LAWS3_FMT_ALWAYS_INLINE value(double val) : double_value(val) {}
  LAWS3_FMT_ALWAYS_INLINE value(long double val) : long_double_value(val) {}
  constexpr LAWS3_FMT_ALWAYS_INLINE value(bool val) : bool_value(val) {}
  constexpr LAWS3_FMT_ALWAYS_INLINE value(char_type val) : char_value(val) {}
  LAWS3_FMT_CONSTEXPR LAWS3_FMT_ALWAYS_INLINE value(const char_type* val) {
    string.data = val;
    if (is_constant_evaluated()) string.size = {};
  }
  LAWS3_FMT_CONSTEXPR LAWS3_FMT_ALWAYS_INLINE
  value(basic_string_view<char_type> val) {
    string.data = val.data();
    string.size = val.size();
  }
  LAWS3_FMT_ALWAYS_INLINE value(const void* val) : pointer(val) {}
  LAWS3_FMT_ALWAYS_INLINE value(const named_arg_info<char_type>* args,
                                size_t size)
      : named_args{args, size} {}

  template <typename T>
  LAWS3_FMT_CONSTEXPR20 LAWS3_FMT_ALWAYS_INLINE value(T& val) {
...
template <typename T> struct format_as_result {
  template <typename U, LAWS3_FMT_ENABLE_IF(std::is_enum<U>::value ||
...
#define LAWS3_FMT_MAP_API LAWS3_FMT_CONSTEXPR LAWS3_FMT_ALWAYS_INLINE
...
template <typename Context> struct arg_mapper {
  ...
  LAWS3_FMT_MAP_API auto map(signed char val) -> int { return val; }
  LAWS3_FMT_MAP_API auto map(unsigned char val) -> unsigned { return val; }
  LAWS3_FMT_MAP_API auto map(short val) -> int { return val; }
  LAWS3_FMT_MAP_API auto map(unsigned short val) -> unsigned { return val; }
  LAWS3_FMT_MAP_API auto map(int val) -> int { return val; }
  LAWS3_FMT_MAP_API auto map(unsigned val) -> unsigned { return val; }
  LAWS3_FMT_MAP_API auto map(long val) -> long_type { return val; }
  LAWS3_FMT_MAP_API auto map(unsigned long val) -> ulong_type { return val; }
  LAWS3_FMT_MAP_API auto map(long long val) -> long long { return val; }
  LAWS3_FMT_MAP_API auto map(unsigned long long val) -> unsigned long long {
    return val;
  }
  LAWS3_FMT_MAP_API auto map(int128_opt val) -> int128_opt { return val; }
  LAWS3_FMT_MAP_API auto map(uint128_opt val) -> uint128_opt { return val; }
  LAWS3_FMT_MAP_API auto map(bool val) -> bool { return val; }

  template <typename T, LAWS3_FMT_ENABLE_IF(std::is_same<T, char>::value ||
                                            std::is_same<T, char_type>::value)>
  LAWS3_FMT_MAP_API auto map(T val) -> char_type {
    return val;
  }
  template <typename T, enable_if_t<(std::is_same<T, wchar_t>::value ||
                                     ...
                                     std::is_same<T, char32_t>::value) &&
                                        !std::is_same<T, char_type>::value,
                                    int> = 0>
  LAWS3_FMT_MAP_API auto map(T) -> unformattable_char {
    return {};
  }

  LAWS3_FMT_MAP_API auto map(float val) -> float { return val; }
  LAWS3_FMT_MAP_API auto map(double val) -> double { return val; }
  LAWS3_FMT_MAP_API auto map(long double val) -> long double { return val; }

  LAWS3_FMT_MAP_API auto map(char_type* val) -> const char_type* { return val; }
  LAWS3_FMT_MAP_API auto map(const char_type* val) -> const char_type* {
    return val;
  }
  template <typename T, typename Char = char_t<T>,
            LAWS3_FMT_ENABLE_IF(std::is_same<Char, char_type>::value &&
                                !std::is_pointer<T>::value)>
  LAWS3_FMT_MAP_API auto map(const T& val) -> basic_string_view<Char> {
    return to_string_view(val);
  }
  template <typename T, typename Char = char_t<T>,
            LAWS3_FMT_ENABLE_IF(!std::is_same<Char, char_type>::value &&
                                !std::is_pointer<T>::value)>
  LAWS3_FMT_MAP_API auto map(const T&) -> unformattable_char {
    return {};
  }

  LAWS3_FMT_MAP_API auto map(void* val) -> const void* { return val; }
  LAWS3_FMT_MAP_API auto map(const void* val) -> const void* { return val; }
  LAWS3_FMT_MAP_API auto map(std::nullptr_t val) -> const void* { return val; }
  ...
  LAWS3_FMT_CONSTEXPR auto map(const T&) -> unformattable_pointer {
    return {};
  }

  template <typename T, std::size_t N,
            LAWS3_FMT_ENABLE_IF(!std::is_same<T, wchar_t>::value)>
  LAWS3_FMT_MAP_API auto map(const T (&values)[N]) -> const T (&)[N] {
    return values;
  }

  // Only map owning types because mapping views can be unsafe.
  template <typename T, typename U = format_as_t<T>,
            LAWS3_FMT_ENABLE_IF(std::is_arithmetic<U>::value)>
  LAWS3_FMT_MAP_API auto map(const T& val)
      -> decltype(LAWS3_FMT_DECLTYPE_THIS map(U())) {
  ...
  template <typename T, LAWS3_FMT_ENABLE_IF(formattable<T>::value)>
  LAWS3_FMT_MAP_API auto do_map(T& val) -> T& {
    return val;
  }
  template <typename T, LAWS3_FMT_ENABLE_IF(!formattable<T>::value)>
  LAWS3_FMT_MAP_API auto do_map(T&) -> unformattable {
    return {};
  }

  template <typename T, typename U = remove_const_t<T>,
            LAWS3_FMT_ENABLE_IF((std::is_class<U>::value ||
                                 std::is_enum<U>::value ||
                                 std::is_union<U>::value) &&
                                !has_to_string_view<U>::value &&
                                !is_char<U>::value && !is_named_arg<U>::value &&
                                !std::is_arithmetic<format_as_t<U>>::value)>
  LAWS3_FMT_MAP_API auto map(T& val)
      -> decltype(LAWS3_FMT_DECLTYPE_THIS do_map(val)) {
    return do_map(val);
  }

  template <typename T, LAWS3_FMT_ENABLE_IF(is_named_arg<T>::value)>
  LAWS3_FMT_MAP_API auto map(const T& named_arg)
      -> decltype(LAWS3_FMT_DECLTYPE_THIS map(named_arg.value)) {
...
template <typename T, typename Char>
#if LAWS3_FMT_CLANG_VERSION && LAWS3_FMT_CLANG_VERSION <= 1500
...
template <bool PACKED, typename Context, typename T,
          LAWS3_FMT_ENABLE_IF(PACKED)>
LAWS3_FMT_CONSTEXPR auto make_arg(T& val) -> value<Context> {
...
template <typename Context, typename T>
LAWS3_FMT_CONSTEXPR auto make_arg(T& val) -> basic_format_arg<Context> {
...
template <bool PACKED, typename Context, typename T,
          LAWS3_FMT_ENABLE_IF(!PACKED)>
LAWS3_FMT_CONSTEXPR inline auto make_arg(T& val) -> basic_format_arg<Context> {
...
// `lll::fmt::basic_format_args` for passing into type-erased formatting
// functions such as `lll::fmt::vformat`.
...
struct format_arg_store {
  ...
  template <typename... T>
  LAWS3_FMT_MAP_API format_arg_store(T&... values)
...
template <typename Context> class basic_format_arg {
 private:
  ...
  friend LAWS3_FMT_CONSTEXPR auto detail::make_arg(T& value)
  ...
  */
  ...
  LAWS3_FMT_CONSTEXPR auto visit(Visitor&& vis) -> decltype(vis(0)) {
    switch (type_) {
...
LAWS3_FMT_DEPRECATED LAWS3_FMT_CONSTEXPR auto visit_format_arg(
...
template <typename Context> class basic_format_args {
 ...
 public:
  ...
  /**
   \rst
   Constructs a `basic_format_args` object from `~lll::fmt::format_arg_store`.
   \endrst
   */
  ...
  /**
   ...
   `~lll::fmt::dynamic_format_arg_store`.
   \endrst
   */
...
class context {
 ...
 public:
   ...
   */
  LAWS3_FMT_CONSTEXPR context(iterator out, basic_format_args<context> ctx_args,
  ...
  LAWS3_FMT_CONSTEXPR auto arg_id(string_view name) -> int {
...
/**
  ...
  converted to `~lll::fmt::format_args`. `Context` can be omitted in which case
  it defaults to `~lll::fmt::format_context`. See `~lll::fmt::arg` for lifetime
  considerations.
  \endrst
 */
...
template <typename Context = format_context, typename... T,
          ...
          LAWS3_FMT_ENABLE_IF(NUM_NAMED_ARGS == 0)>
...
/**
  ...
  **Example**::
    ...
    lll::fmt::print("Elapsed time: {s:.2f} seconds", lll::fmt::arg("s", 1.23));
  \endrst
 */
template <typename Char, typename T>
...
LAWS3_FMT_EXPORT using format_args = basic_format_args<format_context>;

// We cannot use enum classes as bit fields because of a gcc bug, so we put them
// in namespaces instead (https://gcc.gnu.org/bugzilla/show_bug.cgi?id=61414).
// Additionally, if an underlying type is specified, older gcc incorrectly warns
// that the type is too small. Both bugs are fixed in gcc 9.3.
#if LAWS3_FMT_GCC_VERSION && LAWS3_FMT_GCC_VERSION < 903
#  define LAWS3_FMT_ENUM_UNDERLYING_TYPE(type)
#else
#  define LAWS3_FMT_ENUM_UNDERLYING_TYPE(type) : type
#endif
namespace align {
enum type LAWS3_FMT_ENUM_UNDERLYING_TYPE(unsigned char){none, left, right,
                                                        center, numeric};
}
using align_t = align::type;
namespace sign {
enum type LAWS3_FMT_ENUM_UNDERLYING_TYPE(unsigned char){none, minus, plus,
                                                        space};
}
...
struct fill_t {
 ...
 public:
  template <typename Char>
  LAWS3_FMT_CONSTEXPR void operator=(basic_string_view<Char> s) {
    auto size = s.size();
    ...
    LAWS3_FMT_ASSERT(size <= max_size, "invalid fill");
    for (size_t i = 0; i < size; ++i) data_[i] = static_cast<char>(s[i]);
  }

  LAWS3_FMT_CONSTEXPR void operator=(char c) {
    data_[0] = c;
    size_ = 1;
  }
...
template <typename Char> struct arg_ref {
  LAWS3_FMT_CONSTEXPR arg_ref() : kind(arg_id_kind::none), val() {}

  LAWS3_FMT_CONSTEXPR explicit arg_ref(int index)
      : kind(arg_id_kind::index), val(index) {}
  LAWS3_FMT_CONSTEXPR explicit arg_ref(basic_string_view<Char> name)
      : kind(arg_id_kind::name), val(name) {}

  LAWS3_FMT_CONSTEXPR auto operator=(int idx) -> arg_ref& {
  ...
  arg_id_kind kind;
  union value {
    LAWS3_FMT_CONSTEXPR value(int idx = 0) : index(idx) {}
    LAWS3_FMT_CONSTEXPR value(basic_string_view<Char> n) : name(n) {}
...
LAWS3_FMT_CONSTEXPR auto find(Ptr first, Ptr last, T value, Ptr& out) -> bool {
...
template <typename Char>
LAWS3_FMT_CONSTEXPR auto parse_nonnegative_int(const Char*& begin,
                                               const Char* end,
...
LAWS3_FMT_CONSTEXPR inline auto parse_align(char c) -> align_t {
  switch (c) {
  case '<':
    return align::left;
  case '>':
...
LAWS3_FMT_CONSTEXPR auto do_parse_arg_id(const Char* begin, const Char* end,
...
template <typename Char> struct dynamic_spec_id_handler {
  ...
  LAWS3_FMT_CONSTEXPR void on_auto() {
...
template <typename Char>
LAWS3_FMT_CONSTEXPR auto parse_precision(const Char* begin, const Char* end,
...
template <typename Char>
LAWS3_FMT_CONSTEXPR auto parse_format_specs(
  ...
  struct {
    ...
    LAWS3_FMT_CONSTEXPR auto operator()(pres pres_type, int set)
        -> const Char* {
      if (!in(arg_type, set)) {
...
template <typename Char, typename Handler>
LAWS3_FMT_CONSTEXPR auto parse_replacement_field(const Char* begin,
                                                 const Char* end,
                                                 Handler&& handler)
    -> const Char* {
  struct id_adapter {
    ...
    LAWS3_FMT_CONSTEXPR void on_auto() { arg_id = handler.on_arg_id(); }
    LAWS3_FMT_CONSTEXPR void on_index(int id) {
...
LAWS3_FMT_CONSTEXPR void parse_format_string(basic_string_view<Char> format_str,
                                             Handler&& handler) {
  ...
  struct writer {
    LAWS3_FMT_CONSTEXPR void operator()(const Char* from, const Char* to) {
      if (from == to) return;
      for (;;) {
...
LAWS3_FMT_CONSTEXPR auto parse_format_specs(ParseContext& ctx)
    -> decltype(ctx.begin()) {
...
template <typename Char, typename... Args> class format_string_checker {
 ...
 public:
  explicit LAWS3_FMT_CONSTEXPR format_string_checker(
  ...
  LAWS3_FMT_CONSTEXPR void on_error(const char* message) {
    report_error(message);
  }
};
...
template <typename..., typename S,
          LAWS3_FMT_ENABLE_IF(!is_compile_string<S>::value)>
LAWS3_FMT_ALWAYS_INLINE void check_format_string(const S&) {
#ifdef LAWS3_FMT_ENFORCE_COMPILE_STRING
  static_assert(
      is_compile_string<S>::value,
      "LAWS3_FMT_ENFORCE_COMPILE_STRING requires all format strings to use "
      "LAWS3_FMT_STRING.");
#endif
}
...
void check_format_string(S format_str) {
  ...
  LAWS3_FMT_CONSTEXPR auto s = basic_string_view<char_t>(format_str);
  using checker = format_string_checker<char_t, remove_cvref_t<Args>...>;
  LAWS3_FMT_CONSTEXPR bool error =
...
LAWS3_FMT_API void vprint_mojibake(FILE*, string_view, format_args,
                                   bool = false);
#ifndef _WIN32
...
struct formatter<T, Char,
 ...
 public:
  ...
  template <detail::type U = detail::type_constant<T, Char>::value,
            LAWS3_FMT_ENABLE_IF(U == detail::type::string_type ||
...
template <typename Char, typename... Args> class basic_format_string {
 ...
 public:
  ...
  LAWS3_FMT_CONSTEVAL LAWS3_FMT_ALWAYS_INLINE basic_format_string(const S& s)
      : str_(s) {
    static_assert(
        detail::count<
  ...
  LAWS3_FMT_ALWAYS_INLINE operator basic_string_view<Char>() const {
    return str_;
  }
...
LAWS3_FMT_INLINE auto format_to(OutputIt&& out, format_string<T...> fmt,
  ...
  return vformat_to(LAWS3_FMT_FWD(out), fmt,
...
LAWS3_FMT_INLINE auto format_to_n(OutputIt out, size_t n,
    ...
    -> format_to_n_result<OutputIt> {
  return vformat_to_n(out, n, fmt, lll::fmt::make_format_args(args...));
}
...
struct format_to_result {
  ...
  LAWS3_FMT_CONSTEXPR operator OutputIt&() & noexcept { return out; }
  LAWS3_FMT_CONSTEXPR operator const OutputIt&() const& noexcept { return out; }
  LAWS3_FMT_CONSTEXPR operator OutputIt&&() && noexcept {
...
LAWS3_FMT_INLINE auto format_to(char (&out)[N], format_string<T...> fmt,
  ...
  return vformat_to(out, fmt, lll::fmt::make_format_args(args...));
}
...
/**
  ...
  **Example**::
    ...
    lll::fmt::print("Elapsed time: {0:.2f} seconds", 1.23);
  \endrst
 */
template <typename... T>
...
LAWS3_FMT_END_EXPORT
LAWS3_FMT_GCC_PRAGMA("GCC pop_options")
