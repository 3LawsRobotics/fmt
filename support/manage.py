    ...
    for version in env.versions:
        ...
        for filename in ["reference.rst", "api.rst", "index.rst"]:
            ...
            with rewrite(os.path.join(target_doc_dir, filename)) as b:
                ...
                if version == ("3.0.2"):
                    b.data = b.data.replace("fprintf(std::ostream&", "fprintf(std::ostream &")
                if version == ("5.3.0"):
                    b.data = b.data.replace(
                ...
                if version.startswith("7.1."):
                    ...
                    b.data = b.data.replace("join(It, It", "join(It, Sentinel")
                    b.data = b.data.replace(
                        "lll::fmt::format_to(OutputIt, const S&, Args&&...)",
                        "lll::fmt::format_to(OutputIt, const S&, Args&&...) -> "
        ...
        with rewrite(index) as b:
            b.data = b.data.replace("doc/latest/index.html#format-string-syntax", "syntax.html")
        # Fix issues in syntax.rst.
        index = os.path.join(target_doc_dir, "syntax.rst")
        with rewrite(index) as b:
            b.data = b.data.replace("..productionlist:: sf\n", ".. productionlist:: sf\n ")
        ...
        build.build_docs(
            version, doc_dir=target_doc_dir, include_dir=include_dir, work_dir=env.build_dir
        )
        ...
        for link, target in {"index": "contents", "api": "reference"}.items():
            ...
            target += ".html"
            if os.path.exists(os.path.join(html_dir, target)) and not os.path.exists(link):
...
def release(args):
    ...
    r = requests.post(
        ...
        data=json.dumps(
            {"tag_name": version, "target_commitish": "release", "body": changes, "draft": True}
        ),
    )
