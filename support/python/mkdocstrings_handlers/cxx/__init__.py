...
def get_description(node: ElementTree.Element) -> List[ElementTree.Element]:
    return node.findall("briefdescription/para") + node.findall("detaileddescription/para")
...
def render_decl(d: Definition) -> str:
    ...
    if d.params is not None:
        params = ", ".join([(p.type + " " if p.type else "") + p.name for p in d.params])
...
class CxxHandler(BaseHandler):
    def __init__(self, **kwargs: Any) -> None:
        ...
        _, _ = p.communicate(
            ...
            PREDEFINED       = _WIN32=1 \
                               ...
                               "LAWS3_FMT_DOC=1"
            """.format(
                " ".join([os.path.join(include_dir, h) for h in headers]), self._doxyxml_dir
            ).encode(
                "utf-8"
            )
        )
    ...
    def collect_compound(self, identifier: str, cls: List[ElementTree.Element]) -> Definition:
        ...
        with open(path) as f:
            ...
            d.members = []
            for m in node.findall('sectiondef[@kind="public-attrib"]/memberdef') + node.findall(
                'sectiondef[@kind="public-func"]/memberdef'
            ):
...
def get_handler(theme: str, custom_templates: Optional[str] = None, **_config: Any) -> CxxHandler:
